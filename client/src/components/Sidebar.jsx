  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 flex items-center justify-between bg-slate-900 text-white px-4 shadow z-50">
        <h1 className="font-semibold text-lg truncate">Infra IT</h1>
      ...
      >
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li>
              <Link
                to="/infra"
                onClick={closeSidebar}
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition ${isActive("/s2h")}`}
              >
                <Home size={18} />
            ...
            <li>
              <Link
                to="/infra/todos"
                onClick={closeSidebar}
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition ${isActive("/s2h/test")}`}
              >
                <ListTodo size={18} />
                <span className="truncate">Todo's List</span>
              </Link>
            </li>
              ...
              <AnimatePresence>
                {openMenu === "security" && (
                  ...
                  >
                    ...
                    <li><Link to="/s2h/users/register" className={subLink}>Register</Link></li>
                <li><Link to="/s2h/users" className={subLink}>Get Users</Link></li>
                <li><Link to="/s2h/users/update/role" className={subLink}>Update User Role</Link></li>
                <li><Link to="/s2h/users/status" className={subLink}>Enable / Disable User</Link></li>
        ...
        <div className="border-t border-slate-800 px-6 py-4 text-sm">
          <p className="font-semibold text-white truncate"> {user?.name.toUpperCase()}</p>
          <p className="text-xs text-slate-400 truncate"> Role : {user?.role}</p>
          <p className="text-xs text-slate-400 truncate"> Workgroup :{user?.workgroup}</p>
        </div>
